Unit TERRA_BinarySerialization;

{$I terra.inc}

Interface
Uses TERRA_Object, TERRA_ObjectTree, TERRA_String, TERRA_Utils, TERRA_Stream, TERRA_FileStream;

Const
  TERRABinaryHeader = 'TbXD';

Type
  TERRABinaryReader = Class(TERRAObject)
    Protected
      _Keys:Array Of TERRAString;
      _KeyCount:Integer;

      Procedure LReadNode(Src:TERRAStream; Node:TERRAObjectNode);

    Public
      Function Load(SourceFile:TERRAString):TERRAObjectNode;
  End;

  TERRABinaryWriter = Class(TERRAObject)
    Protected
      _Keys:Array Of TERRAString;
      _KeyCount:Integer;
      ...
      Procedure WriteNode(Dest:Stream; Node:TERRAObjectNode);
