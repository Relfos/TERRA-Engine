...
function BTMemoryGetProcAddress(fp_module: PBTMemoryModule;
  const fp_name: PTERRAChar): Pointer;
// free module
procedure BTMemoryFreeLibrary(fp_module: PBTMemoryModule);
// returns last error
function BTMemoryGetLastError:TERRAString;
...
function PTERRACharToPTERRAChar(f_X: Pointer): PTERRAChar;
begin
{$IFDEF UNICODE}
  Result := StringToOleStr(PTERRAChar(f_X));
{$ELSE}
  Result := PTERRAChar(f_X);
{$ENDIF}
end;
...
begin
  ...
  begin
    ...
    begin
      l_handle := LoadLibrary(PTERRACharToPTERRAChar(IncF(fp_module^.codeBase,
            lp_importDesc^.Name)));
      if (l_handle = INVALID_HANDLE_VALUE) then
      begin
        lastErrStr :=
          'BuildImportTable: can''t load library: ' + PTERRACharToPTERRAChar
...
begin
  ...
  begin
    if StrComp(fp_name, PTERRACharToPTERRAChar(IncF(fp_module^.codeBase, l_nameRef^))
      ) = 0 then
    begin
      l_idx := l_ordinal^;
      break;
