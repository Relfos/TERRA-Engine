...

{
  ...
  2006-07-25   BUG 1     - There was a memory bug in the main component
                           Procedure (fixed thanks to Steven L Brenner)
...
const
  {ZLIB constants}
  ZLIBErrors: Array[-6..2] Of TERRAString = ('incompatible version (-6)',
...
type
  {$IFNDEF UseDelphi}
    ...
    Exception = class(TObject)
      constructor Create(Msg:TERRAString);
    end;
...
type
  ...
  {$IFNDEF UseDelphi}
    ...
    public
      ...
      constructor Create(Filename:TERRAString; Mode: TFileStreamModeSet);
      Procedure Release; override;
    end;

    {Stream for reading from resources}
    TResourceStream = class(TStream)
      constructor Create(Instance: HInst; const ResName:TERRAString; ResType:PTERRAChar);
    private
  ...
  private
    ...
    {Returns the version}
    function GetLibraryVersion:TERRAString;
  protected
    {Being created}
  ...
  public
    ...
    procedure AddtEXt(const Keyword, Text:TERRAString);
    procedure AddzTXt(const Keyword, Text:TERRAString);
    {$IFDEF UseDelphi}
    ...
    {$ENDIF}
    {Calling errors}
    procedure RaiseError(ExceptionClass: ExceptClass; Text:TERRAString);
    ...
    Procedure Release; override;
    {$IFNDEF UseDelphi}procedure LoadFromFile(const Filename:TERRAString);{$ENDIF}
    {$IFNDEF UseDelphi}procedure SaveToFile(const Filename:TERRAString);{$ENDIF}
    procedure LoadFromStream(Stream: TStream);
      {$IFDEF UseDelphi}override;{$ENDIF}
    procedure SaveToStream(Stream: TStream); {$IFDEF UseDelphi}override;{$ENDIF}
    {Loading the image from resources}
    procedure LoadFromResourceName(Instance: HInst; const Name:TERRAString);
    ...
    {$IFNDEF UseDelphi}property Palette: HPalette read GetPalette write
      SetPalette;{$ENDIF}
    {Returns the version}
    property Version:TERRAString read GetLibraryVersion;
  end;

  {Chunk name object}
  TChunkName = Array[0..3] Of TERRAChar;
  ...
  private
    ...
    class function GetName:TERRAString; virtual;
    {Returns the chunk name}
    function GetChunkName:TERRAString;
  public
    ...
    property Name:TERRAString read GetChunkName;
  ...
  protected
    ...
    procedure CopyNonInterlacedRGB8(
      Src, Dest, Trans{$IFDEF Store16bits}, Extra{$ENDIF}: PTERRAChar);
  ...
  protected
    ...
    procedure EncodeNonInterlacedRGB8(Src, Dest, Trans: PTERRAChar);
    procedure EncodeNonInterlacedRGB16(Src, Dest, Trans: PTERRAChar);
    procedure EncodeNonInterlacedGrayscale16(Src, Dest, Trans: PTERRAChar);
    procedure EncodeNonInterlacedPalette148(Src, Dest, Trans: PTERRAChar);
    procedure EncodeNonInterlacedRGBAlpha8(Src, Dest, Trans: PTERRAChar);
    procedure EncodeNonInterlacedRGBAlpha16(Src, Dest, Trans: PTERRAChar);
    procedure EncodeNonInterlacedGrayscaleAlpha8(Src, Dest, Trans: PTERRAChar);
    procedure EncodeNonInterlacedGrayscaleAlpha16(Src, Dest, Trans: PTERRAChar);
    procedure EncodeInterlacedRGB8(const Pass: Byte; Src, Dest, Trans: PTERRAChar);
    procedure EncodeInterlacedRGB16(const Pass: Byte; Src, Dest, Trans: PTERRAChar);
    procedure EncodeInterlacedPalette148(const Pass: Byte;
      Src, Dest, Trans: PTERRAChar);
  ...
  private
    fKeyword, fText:TERRAString;
  public
    {Keyword and text}
    property Keyword:TERRAString read fKeyword write fKeyword;
    property Text:TERRAString read fText write fText;
    {Loads the chunk from a stream}
    function LoadFromStream(Stream: TStream; const ChunkName: TChunkName;
      Size: Integer): Boolean; override;
    {Saves the chunk to a stream}
...
{$IFNDEF UseDelphi}
  function FileExists(Filename:TERRAString): Boolean;
  var
    FindFile: THandle;
    FindData: TWin32FindData;
  begin
    FindFile := FindFirstFile(PTERRAChar(Filename), FindData);
...
{$IFNDEF UseDelphi}
  ...
  constructor Exception.Create(Msg:TERRAString);
  begin
  end;
{$ENDIF}
...
function DecompressZLIB(const Input: Pointer; InputSize: Integer;
  ...
  var ErrorOutput:TERRAString): Boolean;
var
...
Procedure TPngPointerList.Release;
begin
...
function TStream.GetSize: Longint;
  ...
  constructor TFileStream.Create(Filename:TERRAString; Mode: TFileStreamModeSet);
    {Makes file mode}
  ...
  begin
    ...
    {Create handle}
    fHandle := CreateFile(PTERRAChar(Filename), OpenMode, FILE_SHARE_READ or
  ...
  constructor TResourceStream.Create(Instance: HInst; const ResName:TERRAString;
    ResType: PTERRAChar);
  var
    ResID: HRSRC;
    ResGlobal: HGlobal;
  begin
    ...
    ResID := FindResource(hInstance, PTERRAChar(ResName), RT_RCDATA);
...
function TChunk.GetChunkName:TERRAString;
begin
  Result := fName
end;

{Returns the chunk name 2}
class function TChunk.GetName:TERRAString;
begin
...
begin
  ...
  fKeyword := PTERRAChar(Data);  {Get keyword and compression method bellow}
  ...
  begin
    Output := nil;
    if DecompressZLIB(PTERRAChar(Longint(Data) + Length(fKeyword) + 2),
...
var
  ProcResult : Integer;
  IDATHeader : Array[0..3] Of TERRAChar;
  IDATCRC    : Cardinal;
begin
...
begin
  ...
  Dest := PTERRAChar(Longint(Dest) + Col * 3);
  repeat
    {Copy this row}
...
begin
  ...
  repeat
    ...
    repeat
      ...
      Dest2 := PTERRAChar(Longint(Dest) + (Header.BitDepth * Col) div 8);
      {Copy data}
...
procedure TChunkIDAT.CopyInterlacedPalette2(const Pass: Byte; Src, Dest,
  Trans{$IFDEF Store16bits}, Extra{$ENDIF}: PTERRAChar);
var
  CurBit, Col: Integer;
  Dest2: PTERRAChar;
begin
  ...
  repeat
    ...
    repeat
      {Adjust pointer to pixel byte bounds}
      Dest2 := PTERRAChar(Longint(Dest) + Col div 2);
      {Copy data}
...
begin
  {Get first column and enter in loop}
  Col := ColumnStart[Pass];
  Dest := PTERRAChar(Longint(Dest) + Col);
  repeat
    {Copy this row}
    Dest^ := Src^; inc(Dest);
    {$IFDEF Store16bits}
    Extra^ := PTERRAChar(Longint(Src) + 1)^; inc(Extra);
    {$ENDIF}
...
begin
  ...
  Col := ColumnStart[Pass];
  Dest := PTERRAChar(Longint(Dest) + Col * 3);
  Trans := PTERRAChar(Longint(Trans) + Col);
  repeat
    {Copy this row and alpha value}
    Trans^ := PTERRAChar(Longint(Src) + 3)^;
    Byte(Dest^)  := fOwner.GammaTable[pByte(Longint(Src) + 2)^]; inc(Dest);
    Byte(Dest^)  := fOwner.GammaTable[pByte(Longint(Src) + 1)^]; inc(Dest);
    Byte(Dest^)  := fOwner.GammaTable[pByte(Longint(Src)    )^]; inc(Dest);
...
begin
  ...
  repeat
    {Copy this row and alpha value}
    Trans^ := PTERRAChar(Longint(Src) + 6)^;
...
var
  ...
  CurrentRow: Integer;
  Trans, Data{$IFDEF Store16bits}, Extra{$ENDIF}: PTERRAChar;
  CopyProc: procedure(const Pass: Byte; Src, Dest,
    Trans{$IFDEF Store16bits}, Extra{$ENDIF}: PTERRAChar) of object;
begin
...
var
  ...
  Trans, Data{$IFDEF Store16bits}, Extra{$ENDIF}: PTERRAChar;
  CopyProc: procedure(
    Src, Dest, Trans{$IFDEF Store16bits}, Extra{$ENDIF}: PTERRAChar) of object;
begin
  CopyProc := nil; {Initialize}
...
const
  IDATHeader: Array[0..3] Of TERRAChar = ('I', 'D', 'A', 'T');
  BUFFER = 5;
...
procedure TChunkIDAT.EncodeNonInterlacedRGB8(Src, Dest, Trans: PTERRAChar);
var
  I: Integer;
begin
  FOR I := 1 TO ImageWidth DO
...
procedure TChunkIDAT.EncodeNonInterlacedRGB16(Src, Dest, Trans: PTERRAChar);
var
  I: Integer;
begin
  FOR I := 1 TO ImageWidth DO
...
procedure TChunkIDAT.EncodeNonInterlacedPalette148(Src, Dest, Trans: PTERRAChar);
begin
  {It's simple as copying the data}
  CopyMemory(Dest, Src, Row_Bytes);
end;
...
procedure TChunkIDAT.EncodeNonInterlacedGrayscale16(Src, Dest, Trans: PTERRAChar);
var
  I: Integer;
begin
  FOR I := 1 TO ImageWidth DO
...
procedure TChunkIDAT.EncodeNonInterlacedRGBAlpha8(Src, Dest, Trans: PTERRAChar);
var
  i: Integer;
begin
...
procedure TChunkIDAT.EncodeNonInterlacedRGBAlpha16(Src, Dest, Trans: PTERRAChar);
var
  i: Integer;
begin
  {Copy the data to the destination, including data from Trans pointer}
...
var
  ...
  Data, Trans: PTERRAChar;
  {Filter used for this line}
  Filter: Byte;
  {Method which will copy the data into the buffer}
  CopyProc: procedure(Src, Dest, Trans: PTERRAChar) of object;
begin
...
begin
  {Get first column and enter in loop}
  Col := ColumnStart[Pass];
  Src := PTERRAChar(Longint(Src) + Col * 3);
  repeat
    {Copy this row}
    Byte(Dest^) := fOwner.InverseGamma[pByte(Longint(Src) + 2)^]; inc(Dest);
    Byte(Dest^) := fOwner.InverseGamma[pByte(Longint(Src) + 1)^]; inc(Dest);
...
begin
  ...
  with Header.BitmapInfo.bmiHeader do
    repeat
      ...
      repeat
        {Adjust pointer to pixel byte bounds}
        Src2 := PTERRAChar(Longint(Src) + (biBitCount * Col) div 8);
        {Copy data}
...
begin
  {Get first column and enter in loop}
  Col := ColumnStart[Pass];
  Src := PTERRAChar(Longint(Src) + Col);
  repeat
    {Copy this row}
...
var
  ...
  PixelsThisRow: Integer;
  CurrentRow : Integer;
  Trans, Data: PTERRAChar;
  CopyProc: procedure(const Pass: Byte;
    Src, Dest, Trans: PTERRAChar) of object;
begin
  CopyProc := nil;  {Initialize to avoid warnings}
  {Defines the method to copy the data to the buffer depending on}
  {the image parameters}
...
{Raises an error}
procedure TPngObject.RaiseError(ExceptionClass: ExceptClass; Text:TERRAString);
begin
...
{$IFNDEF UseDelphi}
  ...
  procedure TPngObject.LoadFromFile(const Filename:TERRAString);
  var
  ...
  procedure TPngObject.SaveToFile(const Filename:TERRAString);
  var
    FileStream: TFileStream;
  begin
...
{Add a text chunk}
procedure TPngObject.AddtEXt(const Keyword, Text:TERRAString);
var
...
{Add a text chunk}
procedure TPngObject.AddzTXt(const Keyword, Text:TERRAString);
var
...
function TPNGObject.GetLibraryVersion:TERRAString;
begin
