...
Interface
Uses TERRA_Math, TERRA_Vector3D, TERRA_Matrix;
...
Type
  ...
  Quaternion=Packed Object
    ...
    {$IFDEF BENCHMARK}
    Procedure TransformSSE(Const M:Matrix);
...
Function QuaternionMatrix(Const Quaternion:Quaternion):Matrix;

Function QuaternionFromRotationMatrix(Const M:Matrix):Quaternion;
...
{$IFDEF SSE}
{$IFDEF BENCHMARK}
Procedure Quaternion.TransformSSE(Const M:Matrix); Register;
{$ELSE}
Procedure Quaternion.Transform(Const M:Matrix); Register;
{$ENDIF}
Asm
  // copy vector, matrix and result vector to registers
  mov ecx, eax
  mov edx, M
...
{$IFNDEF SSE}
Procedure Quaternion.Transform(Const M:Matrix); {$IFDEF FPC}Inline;{$ENDIF}
Var
  QX,QY,QZ,QW:Single;
{$IFDEF NEON_FPU}
  Temp:Quaternion;
