...
procedure AddTimeStampInformation(const I:AnsiString);
{Logs the given information as associated with the current time stamp
Requires that MemCheck is active}

procedure LogSevereExceptions(const WithVersionInfo:AnsiString);
...
var
	MemCheckLogFileName:AnsiString = '';	//The file memcheck will log information to
...
function CallStackTextualRepresentation(const S: TCallStack; const LineHeader:AnsiString):AnsiString;
//Will contain CR/LFs

Var
	RaiseExceptionsOnEnd:Boolean = True;
...
type
	...
	TStringsArray = array[0..99999999] Of AnsiString;
...
type
	...
	public
		...
		function LineWhichContainsAddress(const Address: Cardinal):AnsiString;
	end;
...
var
	...
	UnitsCount: integer;
	OutputFileHeader:AnsiString = 'MemCheck version 2.73'#13#10;
	HeapStatusSynchro : TSynchroObject;

{$IFDEF USE_JEDI_JCL}
function PointerToDebugInfo(Addr: Pointer):AnsiString; //!! by ray
var
...
const
	TObjectVirtualMethodNames: array[1..8] Of AnsiString = ('SafeCallException', 'AfterConstruction', 'BeforeDestruction', 'Dispatch', 'DefaultHandler', 'NewInstance', 'FreeInstance', 'Destroy');
...
function MemoryBlockDump(Block: PMemoryBlocHeader):AnsiString;
const
	MaxDump = 80;
var
	i,
		...
		begin
			if CallerIsNewInstance then
				...
				begin	//Neither an object nor a string, this is a MUser
					...
					PMemoryBlockFooter(PAnsiChar(Result) + SizeOf(TMemoryBlocHeader) + Size)^ := EndOfBlock;
				end;
											  ...
											  if not MemoryBlockFreed(Block) then
													  ...
													  begin
															  case Block.KindOfBlock of
																	  MClass:
																			  S := TObject(PAnsiChar(Block) + SizeOf(TMemoryBlocHeader)).ClassName;
																	  MUser:
																			  S := 'User';
																	  MReallocedUser:
																			  S := 'Realloc';
															  ...
															  if (BlocksToShow[Block.KindOfBlock]) and ((Block.KindOfBlock <> MClass) or (TObject(PAnsiChar(Block) + SizeOf(TMemoryBlocHeader)) is InstancesConformingToForReporting)) then
																	  try
													  ...
													  {this is not a leak}
													  if CheckWipedBlocksOnTermination and (Block.AllocatedSize > 5) and (Block.AllocatedSize <= DoNotCheckWipedBlocksBiggerThan) and (not IsMemFilledWithChar(PAnsiChar(Block) + SizeOf(TMemoryBlocHeader) + 4, Block.AllocatedSize - 5, CharToUseToWipeOut)) then
															  begin
																	  try
...
begin
	...
	UnitsInfo:= PackageInfo(pointer(Pointer(PAnsiChar(@AllocMemSize) + 31 * 4 + 8)^));	//Hacky, no ? I learnt to count on my fingers ! (this stuff is not exported by system.pas)
	{$ELSE}
	UnitsInfo := PInitContext(PAnsiChar(@AllocMemSize) + 128).InitTable;
	{$ENDIF}
		...
		begin
			...
			if {$IFNDEF DELPHI6_OR_LATER}@{$ENDIF}CurrentUnitInfo.Init = @System.System then
				...
				if {$IFNDEF DELPHI6_OR_LATER}@{$ENDIF}CurrentUnitInfo.Init = @SysUtils.SysUtils then
					...
					{$ENDIF}
						if {$IFNDEF DELPHI6_OR_LATER}@{$ENDIF}CurrentUnitInfo.Init = Pointer(PAnsiChar(@Dummy) + DummyToFinalizationOffset) then
	...
	for i:= 0 to NewUnitsInfoOrder.Count - 1 do
		begin
			WriteProcessMemory(ProcessHandle, Pointer(PAnsiChar(@UnitsInfo^.UnitInfo^[0]) + i * SizeOf(PackageUnitEntry)), NewUnitsInfoOrder[i], SizeOf(PackageUnitEntry), BytesWritten);
...
function RoutineWhichContainsAddress(const Address: Cardinal):AnsiString;
var
	Start, Finish, Pivot: integer;
begin
	Start := 0;
	...
	if ExceptProc <> @MyExceptProc then
		...
		begin
			try
				...
				GetModuleFileName(0, PAnsiChar(LogFileName), FileNameBufSize);
...
constructor TUnitDebugInfos.Create(const AName:AnsiString; const NbLines: Cardinal);
begin
	Name := AName;
...
function TUnitDebugInfos.LineWhichContainsAddress(const Address: Cardinal):AnsiString;
var
				...
				begin
					...
					if srcfile^._nName > 0 then
						...
						begin
							...
							CurrentUnit := TUnitDebugInfos.Create(ExtractFileName(PAnsiChar(NameTbl^[srcfile^._nName - 1])), srcln^._cPair);
							AddUnit(CurrentUnit);
...
type
	...
	public
		...
		property OtherObjectClassName:AnsiString read fOwnerClassName;
...
function CardinalToHexa(i: Cardinal):AnsiString;
const
